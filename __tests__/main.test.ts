...
test('skips validation if excludeLabel matches', () => {
  const excludeTitle = new RegExp('Skip')
  const checker = new Checker({errorSize: 20, warningSize: 10, excludeLabel: 'exclude'})
...
test('does validation if excludeLabel not matches', () => {
  const excludeTitle = new RegExp('NO_VALIDATION')
  const checker = new Checker({errorSize: 20, warningSize: 10, excludeLabel: 'include'})
  const result = checker.check({
    title: 'Skip PR size check',
    labels: ['exclude'],
    files: [{additions: 100, filename: '1.txt'}]
...
test('does validation if excludeLabel is undefined', () => {
